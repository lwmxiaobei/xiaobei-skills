...
DEFAULT_RUNTIME_PYTHON = venv_python(DEFAULT_RUNTIME_ROOT / ".venv")
RUNTIME_INSTALLER = Path(__file__).with_name("install_f5_runtime.py")
...
def install_fixed_runtime() -> Path:
    if not RUNTIME_INSTALLER.is_file():
        raise SystemExit(f"F5 TTS runtime installer is missing: {RUNTIME_INSTALLER}")
    print(
        f"F5 TTS {F5_TTS_VERSION} is not installed in the fixed runtime. "
        f"Installing to: {DEFAULT_RUNTIME_ROOT}",
        file=sys.stderr,
    )
    result = subprocess.run(
        [sys.executable, str(RUNTIME_INSTALLER)],
        check=False,
    )
    if result.returncode != 0 or not python_has_f5(DEFAULT_RUNTIME_PYTHON):
        raise SystemExit(
            "F5 TTS automatic installation failed. Run the installer directly for "
            f"details: python3 {RUNTIME_INSTALLER}"
        )
    return DEFAULT_RUNTIME_PYTHON


def ensure_f5_runtime(args: argparse.Namespace) -> None:
    ffmpeg = Path(require_command("ffmpeg"))
    os.environ["PATH"] = str(ffmpeg.parent) + os.pathsep + os.environ.get("PATH", "")
    candidate, checked = discover_f5_python(args)
    if candidate is None:
        if args.no_install_runtime:
            checked_text = "\n".join(f"  {path}" for path in checked)
            raise SystemExit(
                "No Python environment with f5_tts was found and automatic installation "
                "was disabled. Checked:\n"
                f"{checked_text}"
            )
        candidate = install_fixed_runtime()
...
def main() -> int:
    ...
    parser.add_argument(
        "--no-install-runtime",
        action="store_true",
        help="Do not install the fixed F5 TTS runtime when discovery fails",
    )
    parser.add_argument(
        "--check-runtime",
        action="store_true",
    ...
    if args.check_runtime:
        ...
        print(f"Fixed runtime: {DEFAULT_RUNTIME_ROOT}")
        return 0
    ...
    with tempfile.TemporaryDirectory(prefix="media-montage-f5-") as temp_dir:
        temp = Path(temp_dir)
