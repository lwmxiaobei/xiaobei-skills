#!/usr/bin/env python3
"""Validate, install dependencies when needed, and render a videogen project."""
...
def find_command(name: str) -> Path | None:
    ...
    if direct and (name == "node" or (Path(direct).parent / "node").is_file()):
        return Path(direct)
    ...
    return next(
        (
            ...
            if path.is_file() and (name == "node" or (path.parent / "node").is_file())
        ),
        None,
    )
...
def main() -> int:
    ...
    if not (project / "package.json").is_file():
        raise SystemExit(f"Not a videogen project: {project}")
    ...
    if not remotion.is_file():
        ...
        run([str(npm), "install", "--no-audit", "--no-fund"], project, env)
    if not remotion.is_file():
